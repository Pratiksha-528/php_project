...
<form action="emp.php" method="post">
    ...
    <?php
    // Fetch available states for the dropdown
    ...
    <div id="intern_salary_fields" style="display: <?= $selectednature_of_employment == 'Intern' ? 'block' : 'none' ?>;">
        <label for="probation_period_ctc_min">Probation Period CTC (Min):</label>
        <input type="number" name="probation_period_ctc_min" id="probation_period_ctc_min" value="<?= $selectedprobation_period_ctc_min ?>"><br><br>

        <label for="probation_period_ctc_max">Probation Period CTC (Max):</label>
...
if ($result->num_rows > 0) {
    ...
    if ($role == 'admin') {
        // Admin can do both edit and delete
        echo "<div class='edit'><a href='edit_education.php?id=$employeeId'>Update</a></div>";
        echo "<div class='delete'><a href='delete_education.php?id=$employeeId' onclick='return confirm(\"Are you sure you want to delete this record?\")'>Delete</a></div>";
    } elseif ($role == 'edit_user') {
        // Only show edit button if the user is an editor
        echo "<div class='edit'><a href='edit_education.php?id=$employeeId'>Update</a></div>";
    } elseif ($role == 'delete_user') {
        // Only show delete button if the user is a deleter
        echo "<div class='delete'><a href='delete_education.php?id=$employeeId' onclick='return confirm(\"Are you sure you want to delete this record?\")'>Delete</a></div>";
    }
