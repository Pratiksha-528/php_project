    ...
    for ($i = 0; $i < count($files['name']); $i++) {
        ...
        if ($file['error'] == UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($file['tmp_name'], $filepath)) {
                ...
                $sql = "INSERT INTO document (`Employee ID`, `filename`, `filepath`, `uploaded_at`) VALUES (?, ?, ?, NOW())";
...
<form action="upload_document.php?id=<?php echo $employee_id; ?>" method="POST" enctype="multipart/form-data">
...
<ul id="document-list">
    ...
    $sql = "SELECT * FROM document WHERE `Employee ID` = ?";
    $stmt = $conn->prepare($sql);
...
<script>
    ...
    function deleteDocument(filePath) {
        if (confirm("Are you sure you want to delete this document?")) {
            window.location.href = "upload_document.php?id=<?php echo $employee_id; ?>&delete=" + encodeURIComponent(filePath);
        }
    }
</script>
